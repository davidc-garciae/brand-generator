...
export default class AuthService {
  static async register(payload: RegisterPayload): Promise<User> {
    ...
    if (!res.ok) {
      throw new Error(
        data.message || "Registration failed due to an unknown error."
      );
    }
  ...
  static async me(): Promise<User> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        errorData.message || "Failed to fetch user due to an unknown error."
      );
    }
  ...
  static async logout(): Promise<void> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        errorData.message || "Logout failed due to an unknown error."
      );
    }
  }
}
